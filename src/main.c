...
/*   Updated: 2019/09/22 16:59:51 by ajulanov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			set_choice(char **av, t_fractol *set)
{
	...
	{
		set->win = mlx_new_window(set->mlx, WIDTH, WIDTH, \
		"Jaan's Fractol: Mandelbrot set");
		mandelbrot_fractal(set);
	}
	else if (ft_strcmp(av[1], "J") == 0)
	{
		set->win = mlx_new_window(set->mlx, WIDTH, WIDTH, \
		"Jaan's Fractol: Julia set");
		julia_fractal(set);
	}
	else if (ft_strcmp(av[1], "S") == 0)
	{
		set->win = mlx_new_window(set->mlx, WIDTH, WIDTH, "DJ: Burning ship");
		burning_fractal(set);
	}
	else if (ft_strcmp(av[1], "H") == 0)
	{
		set->win = mlx_new_window(set->mlx, WIDTH, WIDTH, "DJ: Love");
		heart_fractal(set);
	}
	else
		kill_bill(ERR_USAGE);
...
void		init_win(t_fractol *set)
{
	...
	set->img_ptr = (int*)mlx_get_data_addr(set->img, &set->bpp, \
	&set->sl, &set->endian);
}
